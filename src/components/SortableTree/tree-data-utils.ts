...
export function getDescendantCount({ node, ignoreCollapsed = true }: any): any {
  return (
...
function mapDescendants({
  ...
  if (typeof nextNode.children !== 'function') {
    nextNode.children = nextNode.children.map((child: any, i: number) => {
 ...
 * @return {number} count
 */
export function getVisibleNodeCount({ treeData }: any): any {
  const traverse = (node: { children: any[]; expanded: boolean }): any => {
    if (
      !node.children ||
      node.expanded !== true ||
    ...
    return (
      ...
      node.children.reduce(
        (total: any, currentNode: any) => total + traverse(currentNode),
        0,
      )
    );
  };
 ...
 * @return {Object[]} changedTreeData - The changed tree data
 */
export function toggleExpandedForAll({ treeData, expanded = true }: any): any {
  return map({
    treeData,
    callback: ({ node }: any) => ({ ...node, expanded }),
...
export function addNodeUnderParent({
  ...
  const changedTreeData = map({
    ...
    getNodeKey,
    ignoreCollapsed,
    callback: ({ node, treeIndex, path }: any) => {
...
function addNodeAtDepthAndIndex({
  ...
  // Get all descendants
  let insertedTreeIndex: any;
  let pathFragment: any = [];
  let parentNode: any = [];
  let childIndex = currentIndex + 1;
  let newChildren = node.children;
  if (typeof newChildren !== 'function') {
    newChildren = newChildren.map((child: any, i: number) => {
...
export function getTreeFromFlatData({
  flatData,
  getKey = (node: { id: any }) => node.id,
  getParentKey = (node: { parentId: any }) => node.parentId,
  rootKey = '0',
}: any): any {
  if (!flatData) {
    return [];
 ...
 * @return {boolean}
 */
export function isDescendant(older: { children: any[] }, younger: any): any {
  return (
    !!older.children &&
 ...
 */
export function getDepth(node: { children: any[] }, depth = 0): any {
  if (!node.children) {
    return depth;
  }
  ...
  return node.children.reduce(
    (deepest: number, child: any) =>
      Math.max(deepest, getDepth(child, depth + 1)),
    depth,
  );
}
